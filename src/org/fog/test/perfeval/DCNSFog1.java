 ...
 *
 */
public class DCNSFog1 {
	static List<FogDevice> fogDevices = new ArrayList<FogDevice>();
	static List<Sensor> sensors = new ArrayList<Sensor>();
	static List<Actuator> actuators = new ArrayList<Actuator>();
	static int numOfAreas = 4;
	static int numOfCamerasPerArea = 7;
	...
	private static void createFogDevices(int userId, String appId) {
		...
		for(i=0;i<numOfAreas;i++){
			...
			if(c==true && (i==(j-1)%numOfAreas || i==(j+1)%numOfAreas || i==(j+numOfAreas-1)%numOfAreas)) {
				n=1;
			}
			addArea(i+"", userId, appId, proxy.getId(),n);
		}
	}

	private static FogDevice addArea(String id, int userId, String appId, int parentId,int n){
		FogDevice router = createFogDevice("d-"+id, 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
		fogDevices.add(router);
		router.setUplinkLatency(2); // latency of connection between router and proxy server is 2 ms
		int c1=numOfCamerasPerArea;
		if(n==1) {
			if(numOfAreas==2) {
				c1=c1+numOfCamerasPerArea;
			}
			else {
				c1=c1+numOfCamerasPerArea1;
				numOfCamerasPerArea1=numOfCamerasPerArea-numOfCamerasPerArea1;
			}
		}
		for(int i=0;i<c1;i++){
